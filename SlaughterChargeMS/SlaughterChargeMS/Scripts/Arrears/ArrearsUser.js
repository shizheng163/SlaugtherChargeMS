...
//初始化表格数据
function InitGrid() {
    var url = '/Arrears/QueryArrearsUserInfo';
    $('#grid').datagrid({
        url: url,
        pagination: true,
        pageSize: 20,
        ...
        toolbar: [{
            ...
            handler: function () {
                ...
                parent.addTab('拖欠客户项目款查看-' + rows[0].Name + '(' + rows[0].UserNo + ')', '/Arrears/Detail?UserId=' + rows[0].Id, 'icon icon-nav');
            }
        }],
        onDblClickRow: function (index, row) {
            parent.addTab('拖欠客户项目款查看-' + row.Name + '(' + row.UserNo + ')', '/Credit/Detail?UserId=' + row.Id, 'icon icon-nav');
        }
    });
