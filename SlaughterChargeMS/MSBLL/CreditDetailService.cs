...
namespace MSBLL
{
    public class CreditDetailService : BaseService<DB_CreditDetail>, ICreditDetailService
    {
        public override void SetCurrentRepository()
        {
            CurrentRepository = db.CreditDetailRepository;
        }

        public void Add(DB_CreditDetail model, out string retMsg)
        {
            retMsg = string.Empty;
            try
            {
        ...
        public void Update(DB_CreditDetail model, out string retMsg)
        {
            retMsg = string.Empty;
            try
            {
        ...
        public DB_CreditDetail GetModel(int Id)
        {
        ...
        public int GetUserArrearsMoney(int UserId)
        {
            try {
                ...
                var query = efdb.DB_CreditDetail.Where(p => p.FK_UserInfo == UserId).AsQueryable();
        ...
        public void Delete(List<int> list, out string retMsg)
        {
            ...
            try
            {
                List<DB_CreditDetail> deleteList = new List<DB_CreditDetail>();
        ...
        {
            var efdb = EfContextFactory.GetCurrentDbContext() as Entities;
            var query = efdb.DB_CreditDetail.GroupBy(p => p.FK_UserInfo);
        ...
        {
            var efdb = EfContextFactory.GetCurrentDbContext() as Entities;
            IQueryable<DB_CreditDetail> query = efdb.DB_CreditDetail.Include("DB_UserInfo").OrderBy(p=>p.TransTime);
